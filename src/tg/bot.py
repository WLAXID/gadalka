...
HELP = (
    ...
    "<b>Как работает:</b>\n"
    "• Каждые 15 минут просматриваю все активные рынки\n"
...
class GadalkaBot:
    ...
    def _format_scan(self) -> str:
        ...
        if s.get("in_range", 0) == 0:
            lines.append("")
            lines.append(
                "ℹ <i><b>Почему 0 ставок?</b> Большинство активных рынков "
                "сейчас на крайних ценах: явные фавориты (~99¢) и явные "
                "аутсайдеры (~1¢). Стратегия ищет середнячков — это редко: "
                "1-2 раза в день. Жди или загляни через час.</i>"
            )
